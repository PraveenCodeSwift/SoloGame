...
//  Copyright © 2015 Praveen. All rights reserved.
//
...
class StickHeroGameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func initAdmob()  {
     //   GADMobileAds.sharedInstance().requestConfiguration.testDeviceIdentifiers = ["72fa853c86e18e2f1c4f5bcbd176007b"]
    ...
    func doSomething() {
        if interstitial.isReady {
            ...
            print("Ad wasn't ready Ad wasn't ready Ad wasn't ready")
        }
    }
    
    func updateScore()  {
        let highScroreNode = childNode(withName: StickHeroGameSceneChildName.HighScore.rawValue) as! SKLabelNode
        highScroreNode.text = "\(score)"
    }
...
private extension StickHeroGameScene {
    ...
    func loadBackground() {
        guard let _ = childNode(withName: "background") as! SKSpriteNode? else {
            ...
            node.size = CGSize(width: DefinedScreenWidth, height: DefinedScreenHeight)
            addChild(node)
            return
        }
    }
    ...
    func loadScore() {
        let scoreBand = SKLabelNode(fontNamed: "HVDComicSerifPro")
    ...
    func loadHighScoreBackground() {
        let back = SKShapeNode(rect: CGRect(x: 300, y: 1024-200-30, width: 240, height: 140), cornerRadius: 20)
        back.zPosition = StickHeroGameSceneZposition.scoreBackgroundZposition.rawValue
        back.fillColor = SKColor.black.withAlphaComponent(0.3)
        back.strokeColor = SKColor.black.withAlphaComponent(0.3)
        addChild(back)
    ...
    func loadHighScore() {
          let high = UserDefaults.standard.integer(forKey: StoreScoreName)
                 let highScore = SKLabelNode(fontNamed: "HVDComicSerifPro")
                 highScore.name = StickHeroGameSceneChildName.HighScore.rawValue
                 highScore.text = "\(high)"
                 highScore.position = CGPoint(x: 400, y: DefinedScreenHeight / 2 - 160)
                 highScore.fontColor = SKColor.white
                 highScore.fontSize = 50
                 highScore.zPosition = StickHeroGameSceneZposition.scoreZposition.rawValue
                 highScore.horizontalAlignmentMode = .center
                 highScore.verticalAlignmentMode = .center
                 if #available(iOS 11.0, *) {
                     highScore.numberOfLines = 2
                 } else {
    ...
    func loadTip() {
        ...
        tip.text = ""
        tip.position = CGPoint(x: 0, y: DefinedScreenHeight / 2 - 400)
        tip.fontColor = SKColor.blue
        tip.fontSize = 77
    ...
    func loadStacks(_ animate: Bool, startLeftPoint: CGFloat) -> SKShapeNode {
        ...
        let min:Int = Int(StackMinWidth / 5)
        var width:CGFloat = CGFloat(randomInRange(min...max) * 10)
        if width <= 250.0 {
            width = 250.0
        }
        let height:CGFloat = StackHeight
        let stack = SKShapeNode(rectOf: CGSize(width: width, height: height))
        stack.fillColor = SKColor.lightText
        stack.strokeColor = SKColor.lightText
